	...
	public static void addDDDungeons() 
	{
		...
		for(Biome blendedBiome : WBBiomes.biomes)
		{
			for (ConfiguredFeature<?> configuredFeature : blendedBiome.getFeatures(GenerationStage.Decoration.SURFACE_STRUCTURES))
			{
				...
				{
					blendedBiome.addFeature(GenerationStage.Decoration.UNDERGROUND_STRUCTURES, Biome.createDecoratedFeature(WBFeatures.DD_DUNGEON_FEATURE, IFeatureConfig.NO_FEATURE_CONFIG, Placement.NOPE, IPlacementConfig.NO_PLACEMENT_CONFIG));
				}
			}
		}
		...
		WBBiomes.biomes.forEach(blendedBiome -> 
			blendedBiome.addFeature(GenerationStage.Decoration.UNDERGROUND_STRUCTURES, 
					Biome.createDecoratedFeature(WBFeatures.DD_DUNGEON_FEATURE, IFeatureConfig.NO_FEATURE_CONFIG, Placement.NOPE, IPlacementConfig.NO_PLACEMENT_CONFIG)));
