...
public class PerformBiomeBlending {
    public static void setupBiomes() {
        ...
        if (!ConfigBlacklisting.isResourceLocationBlacklisted(BlacklistType.FEATURE, new ResourceLocation("minecraft:end_spike")))
            WBBiomes.biomes.forEach(blendedBiome -> blendedBiome.addFeature(GenerationStage.Decoration.SURFACE_STRUCTURES, Feature.END_SPIKE.withConfiguration(new EndSpikeFeatureConfig(false, ImmutableList.of(), null)).withPlacement(Placement.NOPE.configure(IPlacementConfig.NO_PLACEMENT_CONFIG))));
        ...
        for (GenerationStage.Decoration stage : GenerationStage.Decoration.values()) {
            for (ConfiguredFeature<?, ?> grassyFlowerFeature : FeatureGrouping.SMALL_PLANT_MAP.get(stage)) {
                if (WBBiomes.BLENDED_BIOME.getFeatures(stage).stream().noneMatch(addedConfigFeature -> FeatureGrouping.serializeAndCompareFeature(addedConfigFeature, grassyFlowerFeature))) {
    ...
    private static void addBiomeFeatures(Biome biome) {
        for (Decoration stage : GenerationStage.Decoration.values()) {
            for (ConfiguredFeature<?, ?> configuredFeature : biome.getFeatures(stage)) {
                if (WBBiomes.BLENDED_BIOME.getFeatures(stage).stream().noneMatch(addedConfigFeature -> FeatureGrouping.serializeAndCompareFeature(addedConfigFeature, configuredFeature))) {
                    ...
                    if (configuredFeature.config instanceof DecoratedFeatureConfig) {
                        ...
                        if (insideFeature.feature == Feature.RANDOM_BOOLEAN_SELECTOR) {
                            if (ConfigBlacklisting.isResourceLocationBlacklisted(ConfigBlacklisting.BlacklistType.FEATURE, ((TwoFeatureChoiceConfig) insideFeature.config).field_227285_a_.feature.getRegistryName()) || ConfigBlacklisting.isResourceLocationBlacklisted(ConfigBlacklisting.BlacklistType.FEATURE, ((TwoFeatureChoiceConfig) insideFeature.config).field_227286_b_.feature.getRegistryName())) {
                                continue;
                            }
                        }
                    }
    ...
    private static void addBiomeStructures(Biome biome) {
        for (Structure<?> structure : biome.structures.keySet()) {
            if (WBBiomes.BLENDED_BIOME.structures.keySet().stream().noneMatch(struct -> struct == structure)) {
    ...
    private static void addBiomeCarvers(Biome biome) {
        for (Carving carverStage : GenerationStage.Carving.values()) {
            for (ConfiguredCarver<?> carver : biome.getCarvers(carverStage)) {
                ...
                if (WBBiomes.BLENDED_BIOME.getCarvers(carverStage).stream().noneMatch(config -> config.carver == carver.carver)) {
    ...
    private static void addBiomeNaturalMobs(Biome biome) {
        for (EntityClassification entityClass : EntityClassification.values()) {
            for (SpawnListEntry spawnEntry : biome.getSpawns(entityClass)) {
                ...
                if (ConfigBlacklisting.isResourceLocationBlacklisted(ConfigBlacklisting.BlacklistType.SPAWN, Registry.ENTITY_TYPE.getKey(spawnEntry.entityType))) {
                    continue;
                }

                if (WBBiomes.BLENDED_BIOME.getSpawns(entityClass).stream().noneMatch(spawn -> spawn.entityType == spawnEntry.entityType)) {
                    if (Registry.ENTITY_TYPE.getKey(spawnEntry.entityType) != null && Registry.ENTITY_TYPE.getKey(spawnEntry.entityType).getNamespace().equals("minecraft")) {
    ...
    private static void addBiomeSurfaceConfig(Biome biome) {
        ...
        if (biome.getSurfaceBuilderConfig() instanceof SurfaceBuilderConfig) {
            ...
            ISurfaceBuilderConfig surfaceConfig = biome.getSurfaceBuilderConfig();
