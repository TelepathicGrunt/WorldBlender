...
public class PerformBiomeBlending
{
	...
	if (!ConfigBlacklisting.isResourceLocationBlacklisted(BlacklistType.FEATURE, new ResourceLocation("minecraft:end_spike"))) WBBiomes.biomes.forEach(blendedBiome -> blendedBiome.addFeature(GenerationStage.Decoration.SURFACE_STRUCTURES, Feature.END_SPIKE.withConfiguration(new EndSpikeFeatureConfig(false, ImmutableList.of(), (BlockPos) null)).withPlacement(Placement.NOPE.configure(IPlacementConfig.NO_PLACEMENT_CONFIG))));
	...
	for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
	    ...
	    else if (ConfigBlacklisting.isResourceLocationBlacklisted(ConfigBlacklisting.BlacklistType.BLANKET, biome.getRegistryName())) continue;
	...
	for (Carving carverStage : GenerationStage.Carving.values()) {
		...
		if (!WBBiomes.BLENDED_BIOME.getCarvers(carverStage).stream().anyMatch(config -> config.carver == carver.carver)) {
			...
			if (WBConfig.SERVER.allowVanillaCarvers.get()) WBBiomes.biomes.forEach(blendedBiome -> blendedBiome.addCarver(carverStage, carver));
		    }
	...
	for (EntityClassification entityClass : EntityClassification.values()) {
		...
		if (!WBBiomes.BLENDED_BIOME.getSpawns(entityClass).stream().anyMatch(spawn -> spawn.entityType == spawnEntry.entityType)) {
			...
			if (WBConfig.SERVER.allowVanillaSpawns.get()) WBBiomes.biomes.forEach(blendedBiome -> blendedBiome.addSpawn(entityClass, spawnEntry));
		    }
	...
	if (biome.getSurfaceBuilderConfig() instanceof SurfaceBuilderConfig) {
	    ...
	    ISurfaceBuilderConfig surfaceConfig = (ISurfaceBuilderConfig) biome.getSurfaceBuilderConfig();
